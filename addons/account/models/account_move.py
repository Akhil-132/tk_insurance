...
class AccountMove(models.Model):
    ...
    policy_certificate_no = fields.Char(
        string="Policy/Certificate No",
        compute='_compute_policy_certificate_no',
        store=True,
        translate=True
    )

    @api.depends('insurance_information_id')
    def _compute_policy_certificate_no(self):
        for move in self:
            if move.insurance_information_id and move.insurance_information_id.policy_certificate_no:
                move.policy_certificate_no = move.insurance_information_id.policy_certificate_no
            else:
                move.policy_certificate_no = False
    # === Accounting fields === #
    name = fields.Char(
        string='Number',
    ...
    bill_type = fields.Selection([('vendor', 'Vendor'), ('agent', 'Agent'),('sp','SP BILL'),("revenue", "Revenue Bill")], string="Bill Type")
    company_name = fields.Char(string="Company Name")
    insurance_type = fields.Char(string="Insurance Category")
